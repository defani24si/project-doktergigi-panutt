...
import { FaPlus, FaSearch, FaEye, FaEdit, FaTimes, FaUserAlt, FaTooth, FaCalendarAlt, FaMoneyBillWave, FaNotesMedical } from "react-icons/fa";
...
const EMPTY_FORM = { nama: "", tanggalLahir: "", jenisKelamin: "L", noHp: "", alamat: "", status: "Baru" };
...
export default function Pasien() {
  ...
  const handleSave = (e) => {
    e.preventDefault();
    if (isEdit) {
      setPasienData(pasienData.map(p => p.id === form.id ? { ...form, umur: calculateAge(form.tanggalLahir) } : p));
    } else {
      const newPasien = {
        ...form,
  ...
  return (
    <div className="flex flex-col w-full pb-10">
      <PageHeader title="Manajemen Pasien" breadcrumb="Daftar Pasien" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center space-x-4">
          ...
          <div>
            <p className="text-gray-500 text-sm font-medium">Pasien Baru (Bulan Ini)</p>
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
          ...
          <div className="relative w-full md:w-64">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition"
            />
          </div>
          {/* Filter */}
          <select 
            ...
            className="w-full md:w-40 px-4 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-blue-500 transition"
          >
        ...
        <button
          ...
          className="flex items-center justify-center space-x-2 bg-blue-600 text-white px-5 py-2.5 rounded-xl font-medium hover:bg-blue-700 transition shadow-sm"
        >
          <FaPlus />
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredPasien.length === 0 ? (
                <tr>
                  <td colSpan="7" className="px-6 py-8 text-center text-gray-400">Tidak ada data pasien ditemukan.</td>
                </tr>
              ) : (
                filteredPasien.map((p) => (
                  <tr key={p.id} className="hover:bg-blue-50/50 transition duration-150">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${BADGE_COLORS[p.status] || BADGE_COLORS["Tidak Aktif"]}`}>
                        {p.status}
                      </span>
                    </td>
                    <td className="px-6 py-4">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                ...
                <input required value={form.nama} onChange={e => setForm({ ...form, nama: e.target.value })}
                  className="w-full border border-gray-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition" />
              </div>
              ...
              <div>
                ...
                <textarea required rows="2" value={form.alamat} onChange={e => setForm({ ...form, alamat: e.target.value })}
                  className="w-full border border-gray-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition"></textarea>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Status Pasien</label>
